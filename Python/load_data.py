...
def load_all_dtype_data():
    files2= {
        "Dim_Cidade":pd.read_csv("processed_tables/Dim_Cidade.csv", sep=";"),
        ...
        "Fato_Vendas":pd.read_csv("processed_tables/Fato_Vendas_Lucro.csv", sep=";"),
        #"Fato_Vendas_No_Outliers":pd.read_csv(
            #"processed_tables/Fato_Vendas_RealProfit2.csv", sep=";"),
    }
...
def see_outliers(files):
    ...
    for name, df in files.items():
        ...
        mask_total |= (
            (df_clean["Quantidade"] < lin_inf) |
            (df_clean["Quantidade"] > lin_sup)
        )

        for produto, grupo in df_clean.groupby("ID_Produto"):

            Q1 = grupo["Preco_Unitario"].quantile(0.25)
            Q3 = grupo["Preco_Unitario"].quantile(0.75)
            IQR = Q3 - Q1
            ...
            mask_produto = (
                (df_clean["ID_Produto"] == produto) &
                (
                    (df_clean["Preco_Unitario"] < lin_inf) |
                    (df_clean["Preco_Unitario"] > lin_sup)
                )
            )

            mask_total |= mask_produto

        df_outliers = df_clean[mask_total]

        outliers_dict[name] = df_outliers

        print(f"{name}: {mask_total.sum()} outliers")
...
def clean_data():
    ...
    # outliers = outliers_removal(file_object)
